...
# Настройки берутся из Переменных Окружения (Environment Variables) в Vercel
...
# Функция запроса к нейросети (OpenRouter)
...
# Слэш-команда для Администраторов в группе (/generate_quiz)
...
async def admin_start_quiz(message: Types.Message):
    ...
    poll_msg = await bot.send_poll(
        ...
        is_anonymous=False  # Открытый опрос для ведения статистики
    )
...
# Сбор ответов пользователей в базу MongoDB
@dp.poll_answer()
...
# Просмотр статистики в личных сообщениях у бота (/stats)
